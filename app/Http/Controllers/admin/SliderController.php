...
class SliderController extends Controller
{
    ...
    {
        if ($request->user()->cannot('view', slider::class)) {
    ...
    {
        if ($request->user()->cannot('create', slider::class)) {
            return response()->json(['code' => -1, 'message' => "Bạn không có quyền thêm slider"]);
        } else {
    ...
    public function update(request $request)
    {
        if ($request->user()->cannot('update', slider::class)) {
            return response()->json(['code' => -1, 'message' => 'Bạn không có quyền sửa slider']);
        } else {
            $validator = Validator::make(
                $request->all(),
                [
    ...
    {
        if ($request->user()->cannot('delete', slider::class)) {
            return response()->json(['code' => -1, 'message' => "Bạn không có quyền xóa slider"]);
        } else {
    ...
    {
        $key = $request->key;
        $sliders = slider::where('title', 'like', '%' . $key . '%')->orWhere('descriptions', 'like', "%$key%") ->latest()->get();
        $view = view('admin.slider.main_data', compact('sliders'))->render();
